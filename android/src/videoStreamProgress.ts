import { getVideoPlaybackProgress } from './api'
...
/** 從 Android 原生播放器 SharedPreferences 同步進度至 localStorage（供列表顯示） */
export async function syncVideoStreamProgressFromNative(job: RemoteStreamJob): Promise<void> {
  try {
    const p = await getVideoPlaybackProgress(job.host, job.port, job.relPath)
    if (p.positionMs > 0 || p.durationMs > 0) {
      saveVideoStreamProgress(job, p.positionMs, p.durationMs)
    }
  } catch {
    /* 桌面或未播放過 */
  }
...
export async function syncVideoStreamProgressList(jobs: RemoteStreamJob[]): Promise<void> {
  if (jobs.length === 0) return
  await Promise.all(jobs.map((j) => syncVideoStreamProgressFromNative(j)))
}
