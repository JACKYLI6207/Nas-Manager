...
pub fn read_local_reader_image(page_id: &str) -> anyhow::Result<Vec<u8>> {
    ...
    if entry.starts_with("content://") {
        return Err(anyhow!("需要 AppHandle 讀取 content URI"));
    }
    read_local_reader_image_fs(&source, &entry)
}

fn read_local_reader_image_fs(source: &Path, entry: &str) -> anyhow::Result<Vec<u8>> {
    if !source.exists() {
...
fn is_content_uri(s: &str) -> bool {
    s.starts_with("content://")
}

#[cfg(target_os = "android")]
fn is_content_zip_uri(uri: &str) -> bool {
    let lower = uri.to_lowercase();
    lower.contains(".zip") || lower.contains(".cbz")
}

#[cfg(target_os = "android")]
fn map_android_source(
    item: crate::folder_picker::AndroidReaderSource,
) -> LocalReaderSource {
    let kind = if item.kind.eq_ignore_ascii_case("zip") {
        LocalReaderSourceKind::Zip
    } else {
        LocalReaderSourceKind::Folder
    };
    LocalReaderSource {
        path: item.path,
        label: item.label,
...
pub fn list_local_reader_sources_with_app(
    ...
    #[cfg(target_os = "android")]
    {
        if is_content_uri(folder_path) {
            let picker = crate::folder_picker::folder_picker(app)
                .map_err(|e| anyhow!("{}", e.err_message))?;
            let items = picker
                .list_reader_sources(folder_path)
                .map_err(|e| anyhow!("{}", e.err_message))?;
            let mut sources: Vec<LocalReaderSource> =
                items.into_iter().map(map_android_source).collect();
            sources.sort_by(|a, b| compare_natural(&a.label, &b.label));
            return Ok(sources);
        }
    }
...
#[cfg(target_os = "android")]
pub fn prepare_local_reader_zip(app: &AppHandle, source_uri: &str) -> anyhow::Result<String> {
    if !is_content_uri(source_uri) {
        return Ok(source_uri.to_string());
    }
    let picker = crate::folder_picker::folder_picker(app).map_err(|e| anyhow!("{}", e.err_message))?;
    picker
        .cache_document_to_file(source_uri)
        .map_err(|e| anyhow!("{}", e.err_message))
}

#[cfg(not(target_os = "android"))]
pub fn prepare_local_reader_zip(_app: &AppHandle, source_uri: &str) -> anyhow::Result<String> {
...
pub fn load_local_reader_pages_with_app(
    app: &AppHandle,
    source_path: &str,
    source_kind: Option<LocalReaderSourceKind>,
) -> anyhow::Result<LocalReaderPages> {
    #[cfg(not(target_os = "android"))]
    let _ = app;
    #[cfg(target_os = "android")]
    {
        if is_content_uri(source_path) {
            let picker = crate::folder_picker::folder_picker(app)
                .map_err(|e| anyhow!("{}", e.err_message))?;
            let treat_as_zip = source_kind == Some(LocalReaderSourceKind::Zip)
                || (source_kind.is_none() && is_content_zip_uri(source_path));
            if treat_as_zip {
                let path = if source_path.starts_with('/') {
                    source_path.to_string()
                } else {
                    picker
                        .cache_document_to_file(source_path)
                        .map_err(|e| anyhow!("{}", e.err_message))?
                };
                return load_local_reader_pages(&path);
            }
            let res = picker
                .load_reader_pages_from_uri(source_path)
                .map_err(|e| anyhow!("{}", e.err_message))?;
            return Ok(LocalReaderPages {
                title: res.title,
                pages: res
                    .pages
                    .into_iter()
                    .map(|p| LocalReaderPage {
                        caption: p.caption,
                        page_id: p.page_id,
                    })
                    .collect(),
...
mod tests {
    ...
    fn natural_sort_orders_numeric_segments() {
        ...
        assert_eq!(
            compare_natural("小褲褲筆記 1-19話", "小褲褲筆記 101-102話"),
            Ordering::Less
        );
        assert_eq!(
            compare_natural("小褲褲筆記 20話", "小褲褲筆記 101-102話"),
            Ordering::Less
        );
    }
}
