...
/// Android 上 download_dir 可能為 content:// 或尚未設定，避免對無效路徑呼叫 exists。
fn comic_is_downloaded(app: &AppHandle, title: &str) -> bool {
    let dir = app.get_config().read().download_dir.clone();
    let dir_str = dir.to_string_lossy();
    if dir_str.is_empty() || dir_str.starts_with("content://") {
        return false;
    }
    let safe_title = filename_filter(title);
    if safe_title.is_empty() {
        return false;
    }
    std::fs::metadata(dir.join(&safe_title)).is_ok()
}
    ...
    D: serde::Deserializer<'de>,
{
    ...
    match value {
        ...
        _ => Err(Error::custom("isDownloaded 須為布林或 null")),
    }
}
...
impl ComicInSearch {
    ...
    fn from_title_anchor(
        ...
        let is_downloaded = comic_is_downloaded(app, &title);
