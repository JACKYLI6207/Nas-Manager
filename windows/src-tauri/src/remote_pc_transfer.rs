...
pub async fn transfer_remote_pc_files(
    ...
    transfer_remote_pc_files_desktop(app, host, port, selections, dest_tree_uri).await
}
...
#[cfg(not(target_os = "android"))]
async fn transfer_remote_pc_files_desktop(
    app: &AppHandle,
    host: &str,
    port: u16,
    selections: &[RemotePcTransferSelection],
    ...
    match transfer_remote_pc_files_inner_desktop(
        app,
        host,
        port,
        selections,
...
#[cfg(not(target_os = "android"))]
async fn transfer_remote_pc_files_inner_desktop(
    app: &AppHandle,
    host: &str,
    port: u16,
    selections: &[RemotePcTransferSelection],
    ...
    for selection in selections {
        match list_remote_pc_files(host, port, &selection.path).await {
            ...
            Err(err) => {
                collection_failed.push(RemoteTransferFailedItem {
                    path: selection.path.clone(),
                    error: format!("列出 PC 檔案失敗：{}", err.to_string_chain()),
                });
            }
        }
    }
    ...
    *progress_total = file_count;
    ...
    let cache = app.path().cache_dir().context("取得快取目錄失敗")?;
    let temp_dir = cache.join(format!("gm-remote-transfer-{}", uuid::Uuid::new_v4()));
    tokio::fs::create_dir_all(&temp_dir).await?;
    ...
    for (index, file) in files.iter().enumerate() {
        ...
        *last_remote_path = file.remote_path.clone();
        ...
        let transfer_result: anyhow::Result<u64> = async {
            ...
            let dest_full = dest_root.join(&dest_rel);
            if let Some(parent) = dest_full.parent() {
                tokio::fs::create_dir_all(parent).await.with_context(|| {
                    format!("建立目錄失敗：{}", parent.display())
                })?;
            }
            tokio::fs::copy(&temp_file, &dest_full)
                .await
                .with_context(|| format!("寫入本機失敗：{}", dest_full.display()))?;
            Ok(downloaded)
        }
        .await;
