...
/** 依估算頁高粗跳至書籤（等待目標頁解碼時避免仍停在第一頁） */
export function approxScrollToCheckpoint(
  root: HTMLElement,
  cp: PageCheckpoint,
  estimatedPageHeight: number,
): void {
  const h = Math.max(estimatedPageHeight, 320)
  root.scrollTop = cp.pageIndex * h + cp.offsetRatioInPage * h
}
