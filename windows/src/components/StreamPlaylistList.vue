...
const props = defineProps<{
  ...
  showRemove?: boolean
  fill?: boolean
  /** 每列閱讀進度（例：5/120頁 或 12:34/1:45:00） */
  progressForJob?: (job: StreamListJob) => string | null
  /** 每列進度比例 0–1（時間軸） */
  progressRatioForJob?: (job: StreamListJob) => number | null
  /** 每列是否顯示已開啟標記 */
...
const rows = computed(() =>
  props.jobs.map((job) => ({
    ...
    progressRatio: props.progressRatioForJob?.(job) ?? null,
  })),
)
</script>
    ...
    <li v-for="row in rows" :key="row.key" class="stream-pl-item">
      <button type="button" class="stream-pl-play" @click="emit('play', row.job)">
        ...
        <div
          ...
          class="stream-pl-timeline"
          aria-hidden="true"
        >
          <div
            class="stream-pl-timeline-fill"
...
.stream-pl-timeline-fill {
  height: 100%;
  border-radius: inherit;
  background: linear-gradient(90deg, #3d6ef5, #6eb5ff);
  min-width: 2px;
}
.stream-pl-meta {
  display: block;
