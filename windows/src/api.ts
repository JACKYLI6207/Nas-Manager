...
export type MobileSettings = {
  ...
  externalVideoPlayerPath?: string | null
}
...
export const SYSTEM_DEFAULT_PLAYER_ID = '__system_default__'

export function listVideoPlayerOptions() {
  return invoke<VideoPlayerOption[]>('list_video_player_options')
}

export function setDefaultVideoPlayer(exePath: string | null) {
  return invoke<VideoPlayerOption>('set_default_video_player', { exePath })
}

export function pickExternalVideoPlayerExe() {
  return invoke<string | null>('pick_external_video_player_exe')
}
...
export type VideoStreamPrepProgressEvent = {
  phase: string
  message: string
  bytesDone: number
  bytesTotal: number
...
export async function listenVideoStreamPrepProgress(
  handler: (payload: VideoStreamPrepProgressEvent) => void,
): Promise<import('@tauri-apps/api/event').UnlistenFn> {
  return listen<VideoStreamPrepProgressEvent>('video-stream-prep-progress-event', (ev) =>
    handler(ev.payload),
  )
}
